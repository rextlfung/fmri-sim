...
load(datdir + "rest_rss.mat") % Irss
load(datdir + "2d.mat") % ksp_2d_epi
Irss = flip(Irss,2);
Irss = Irss(:,:,:,13:end); % Discard frames prior to steady state
[Nx,Ny,Nz,Nt] = size(Irss);
...
%% Extact one slices and rescale to [0, 1]
z = 40;
gt = mat2gray(squeeze(Irss(:,:,z,:)));
gt(gt < 0.05) = 0;
smaps = squeeze(smaps(:,:,z,:));
...
amp = 1.5e-2; % amplitude of BOLD activation relative to mean signal
...
phase = exp(ones(Nx,1) * 1i*(1:Ny)/Ny/(2*pi));
img = abs(img) .* phase;
...
for n = 2:5 % Loop over regularization strengths
    fn = datdir + sprintf('rand/rec_llr_5e-%d', n);
    load(fn);
    rec_lr = squeeze(X);
